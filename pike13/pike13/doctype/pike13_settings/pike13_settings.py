...
def check_payment_modes(site, doc):
	...
	try:
		r = requests.get(site.pike13_site_url+"/api/v2/desk/payments/configuration", headers=headers).json()
	except requests.exceptions.HTTPError:
		button_label = frappe.bold(_("Get Access Token"))
		frappe.throw(
			_(
	...
	if (len(not_found_in_erp) > 0):
		site.status = "Please map these mode of payments in Site then sync invoices again: "+(','.join(not_found_in_erp)) 
		site.save()
		frappe.msgprint(
			msg= _(
				"<b style='color:red'>"+site.pike13_site_url+"</b><br>Please map these mode of payments in Site then sync invoices again. <br><br>"+('<br>'.join(not_found_in_erp))
			)
		)
		return False
...
def get_invoices_from_pike13(site, doc):
	...
	frappe.msgprint(
		msg= _(
			"<b style='color:green'>"+site.pike13_site_url+"</b>: Invoices synced successfully!"
		)
	)
...
def insert_invoices_from_pike13(invoices, doc, site):
	...
	for x in invoices:
		...
		if(len(erp_invoices) == 0):
			try:
				r = requests.get(site.pike13_site_url+"/api/v2/desk/invoices/"+str(x.get("id")), headers=headers).json()
			except requests.exceptions.HTTPError:
				button_label = frappe.bold(_("Get Access Token"))
				frappe.throw(
					_(
			...
			print(str(x.get("id"))+":"+curr_invoice.get("invoice_date")+":"+str(erp_customer))
...
def get_erp_customer(id, doc, site):
	...
	if(len(erp_customer) == 0):
		...
		try:
			r = requests.get(site.pike13_site_url+"/api/v2/desk/people/"+str(id), headers=headers).json()
		except requests.exceptions.HTTPError:
			frappe.throw(
				_(
