...
def get_invoices_from_pike13(site, doc):
    ...
    while (1 == 1):
        ...
        print(last_invoice, "last_invoice Print")
...
def insert_invoices_from_pike13(invoices, doc, site):
    ...
    for x in invoices:
        ...
        if (len(erp_invoices) == 0):
            ...
            if curr_invoice.get("total_cents") > 0:
...
def get_erp_customer(id, doc, site):
    ...
    print(erp_customer, "erp_customer List")
...
def get_erp_items(items, doc, site):
    erp_items = []
    for x in items:
        print(site.name, "sites Name ")
        item_detail = x.get("detail")
        print(x.get("name"), "Item Name")
        print(item_detail.get("product_id"), "Item details Product Id")
        items_in_db = frappe.get_all("Item",
                                     filters={
                                         "pike13_item_code": item_detail.get("product_id"),
                                         "pike13_site": site.name
                                     },
                                     fields=['name', 'pike13_item_code']
                                     )
        print(items_in_db, "Database list check")
